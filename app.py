...
if st.session_state.get('analyzed'):
    ...
    if 'df_final' not in st.session_state:
        ...
        # --- [상단 지표 5개 표시] ---
        with f_stock:
            st.metric("가용재고", f"{int(df_temp[p['av']].clip(lower=0).sum()):,} 개")
        with f_sales:
            st.metric("일판매량", f"{int(df_temp['일판매량'].sum()):,} 개")
        with f_reorder:
            st.metric("리오더합", f"{int(df_temp['기존리오더'].sum()):,} 개")
        with f_rec_sum:
            st.metric("발주수량", f"{int(df_temp['권장발주수량'].sum()):,} 개", delta=f"{len(df_temp[df_temp['권장발주수량'] > 0])} 품목")
...
def render_step6():
    ...
    if not lead_time_df.empty:
        avg_vendor_lead = lead_time_df.groupby(v_col)['days'].mean().round(1).reset_index()
        avg_vendor_lead.columns = [v_col, '평균소요(일)']
...
if "db_history" in st.session_state and not st.session_state.db_history.empty:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "비고(처리내역)": st.column_config.TextColumn("비고(처리내역)", width=200), 
