...
# ==========================================================
# --- [5단계: 최종 발주 리스트 (모든 기능 복구 및 증발 차단)] ---
# ==========================================================
if st.session_state.get('analyzed') and st.session_state.df_raw is not None:
    st.divider()
    ...
    # ⭐ 금고(session_state)에서 입력했던 수량 강제 매핑 (이게 있어야 검색해도 유지됨)
    ...
    with f2: 
        # 검색 시 sync 함수 호출하여 현재 수량을 금고에 먼저 집어넣음
        st.text_input("🔍 상품명/옵션 검색", 
                     value=st.session_state.common_search, 
                     key="v5_search_fixed", 
                     on_change=sync_all_and_save_mem)
    with f3: 
        d5_d = st.date_input("🗓️ 발주 기준일", datetime.now(KST).date())

    # [5. 필터링 및 검색 적용]
    df_ns = df_display_v5[~df_display_v5[sold_out_col].astype(str).str.contains('품절', na=False)].copy()
    curr_q = st.session_state.common_search.strip()

    if curr_q:
        df_final_v5 = df_ns[df_ns[item].astype(str).str.contains(curr_q, case=False, na=False) | 
                             df_ns[option].astype(str).str.contains(curr_q, case=False, na=False)]
    else:
        danger_names = df_ns[df_ns['권장수량'] > 0][item].unique()
        df_final_v5 = df_ns[df_ns[item].isin(danger_names)] if m5_f == "🚨 고위험/주의" else df_ns[~df_ns[item].isin(danger_names)]

    df_final_v5 = df_final_v5.sort_values(by=[item, option])
    # 금고 동기화를 위해 현재 화면 인덱스 리스트 저장
    st.session_state.current_v5_index = df_final_v5.index.tolist()

    # [6. 데이터 에디터 및 저장]
    d_map = {"상태분류": "상태", item: "상품명", option: "옵션", v_item: "공급쳐명", avail: "가용재고", "리오더 수량": "리오더중", "추가입력": "추가입력", "권장수량": "권장수량", "3일분": "3일분", "최근입고합계": "최근입고"}
    
    with st.form("v5_final_heavy_form"):
        st.data_editor(
            df_final_v5[list(d_map.keys())].rename(columns=d_map),
        ...
        if st.form_submit_button("✅ 최종 발주 확정 (시트1 합산 저장)", use_container_width=True, type="primary"):
            sync_all_and_save_mem()
            final_dict = st.session_state.add_order_dict
    ...
    with c1:
        if st.button("💾 발주 기록 저장", use_container_width=True):
            log_target = df_display_v5[(df_display_v5['권장수량'] > 0) | (df_display_v5['추가입력'] > 0)].copy()
            if not log_target.empty:
                now_s = datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')
                rows = [[now_s, str(r[item]), str(r[option]), str(r[v_item]), int(r[avail]), int(r['리오더 수량']), int(r['추가입력']), int(r['권장수량'])] for _, r in log_target.iterrows()]
                get_sheet().worksheet("발주기록").append_rows(rows)
                st.success("✅ 기록 완료!")
    with c2:
        df_display_v5['합계'] = df_display_v5['권장수량'] + df_display_v5['추가입력']
        csv_df = df_display_v5[df_display_v5['합계'] > 0].copy()
        if not csv_df.empty:
            st.download_button("📥 발주서 CSV 다운로드", csv_df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), f"발주서_{d5_d.strftime('%m%d')}.csv", use_container_width=True)
