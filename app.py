    ...
    if st.session_state.get('df_raw') is not None:
        ...
        if st.session_state.analyzed:
            ...
            for c in [stock, avail, "리오더 수량", "리오더입고수량", t3day, "일판매량", "권장발주량"]:
                if c in df_work_view.columns: df_work_view[c] = df_work_view[c].astype(str)
            ...
            # 5단계 날짜(hist_date_5) 기준 입고수량 다시 매핑
            if hist_date_5 != hist_date_4:
                t_hist_5 = past_hist[(past_hist['날짜'] == hist_date_5.strftime("%Y-%m-%d")) & (past_hist['구분'] == "입고")]
                in_map_5 = t_hist_5.groupby('k_tmp')['수량'].sum().to_dict() if not t_hist_5.empty else {}
                to_order['리오더입고수량'] = to_order['unique_key'].map(in_map_5).fillna(0).astype(int)

            to_order['과거입고수량'] = to_order['리오더입고수량']
            to_order['추가발주수량'] = 0 # 입력 전용 컬럼
            to_order['최종발주량'] = to_order['권장발주량']
            ...
            if not df_final.empty:
                ...
                for c in [avail, "리오더 수량", "과거입고수량", "권장발주량", "최종발주량"]:
                ...
                # 추가발주수량은 입력을 위해 텍스트 컬럼으로 명시
                config_5["추가발주수량"] = st.column_config.TextColumn("추가발주수량(입력 시 리오더 합산)")
                
                def on_edit_5():
                    edits = st.session_state["final_editor_v5"]["edited_rows"]
                    for r_idx_str, change in edits.items():
                        if "추가발주수량" in change:
                            idx = int(r_idx_str); orig_idx = df_final.index[idx]
                            try:
                                ...
                                if add_qty > 0:
                                    ...
                                    # 2. 히스토리 기록
                                    save_history_to_gsheet(pd.DataFrame([[df_final.at[orig_idx, item], df_final.at[orig_idx, option], add_qty]], columns=['상품명', '옵션', '수량']), log_type="추가발주")
                                    # 3. 구글 시트 즉시 저장 (데이터 보존)
                                    save_reorder_data(st.session_state.df_raw[[item, option, '리오더 수량']].rename(columns={item:'상품명', option:'옵션'}))
                            except: pass
                    st.rerun()
                ...
                if c_b1.button("💾 구글 시트에 최종 발주 기록 저장", use_container_width=True):
                    save_df = df_final[df_final['권장발주량'] > 0][[item, option, '권장발주량']] # 권장수량 기준 저장
                    if save_history_to_gsheet(save_df, log_type="발주"): st.success("✅ 발주 기록 저장 완료!")
