...
st.set_page_config(layout="wide", page_title="저스트원 v6.5")
...
if up_file:
    ...
    # 분석 버튼 실행 시 모든 데이터와 매핑 정보를 세션에 꽉 묶어서 저장합니다.
    if st.button("📊 분석 실행", type="primary", use_container_width=True):
        ...
        # [핵심] 현재 시점의 매핑 정보를 데이터와 함께 저장 (이후 매핑을 바꿔도 표가 안 깨지게 함)
        st.session_state.final_mapping = {
            'vn': vn, 'it': it, 'op': op, 'av': av, 't3': t3, 'reg': reg
        }
    ...
    if 'analyzed_data' in st.session_state and 'final_mapping' in st.session_state:
        ...
        # 사장님이 요청하신 순서대로 컬럼 리스트업
        display_cols = [
            m['vn'], m['it'], m['op'], m['av'], '리오더 수량', 
            '입고차감', '추가발주', m['t3'], '1일 판매량', '권장발주수량', '메모'
        ]
        
        # 데이터프레임에 실제 컬럼이 있는지 최종 확인 (방어 코드)
        existing_cols = [c for c in display_cols if c in st.session_state.analyzed_data.columns]
        
        edited_df = st.data_editor(
            st.session_state.analyzed_data[existing_cols],
            use_container_width=True, hide_index=True, key="main_editor",
            column_config={
                "입고차감": st.column_config.NumberColumn("📥 입고(-)", help="리오더 입고 시 마이너스 입력"),
                "추가발주": st.column_config.NumberColumn("➕ 추가발주", help="추가 발주 수량"),
        ...
        if st.button("💾 변경사항 일괄 저장", type="primary", use_container_width=True):
            ...
            if not to_save.empty:
                with st.spinner("📡 구글 시트에 기록 중..."):
                    ...
                    # 저장 후 분석표 삭제 (중복 저장 방지)
                    del st.session_state.analyzed_data
                    st.rerun()
            else:
                st.warning("⚠️ 저장할 데이터(입고차감 또는 추가발주)가 없습니다.")
