...
def save_reorder_data(new_work_df):
    """[핵심 추가] 기존 시트 데이터를 읽어와서 현재 업체 데이터만 업데이트하고 나머지는 보존"""
    try:
...
# --- [탭 구성] ---
tab1, tab2 = st.tabs(["📊 재고 관리 분석", "📜 입출고 기록 조회"])
...
with tab1:
    ...
    uploaded_file = st.file_uploader("엑셀 또는 CSV 파일을 올려주세요", type=['xlsx', 'xls', 'csv'])
    ...
    if st.session_state.get('df_raw') is not None:
        ...
        # --- [1~3단계: 설정] ---
        st.subheader("⚙️ 1~3단계: 매핑 및 분석 설정")
        with st.expander("설정 열기/닫기", expanded=not st.session_state.analyzed):
            c1, c2 = st.columns(2)
            sold_out = c1.selectbox("품절 여부 열", cols, index=find_idx(cols, ['품절']))
            vendor = c1.selectbox("공급처 열", cols, index=find_idx(cols, ['공급처']))
            item = c1.selectbox("상품명 열", cols, index=find_idx(cols, ['상품명']))
            option = c1.selectbox("옵션 열", cols, index=find_idx(cols, ['옵션']))
            avail = c2.selectbox("가용재고 열", cols, index=find_idx(cols, ['가용재고']))
            t3day = c2.selectbox("3일 발주 열", cols, index=find_idx(cols, ['3일']))
            t7day = c2.selectbox("7일 발주 열", cols, index=find_idx(cols, ['7일']))
    ...
    if st.session_state.get('analyzed'):
        ...
        if not hist_df.empty and '구분' in hist_df.columns:
            ...
            if not in_hist.empty:
                ...
                df_work['과거 리오더입고수량'] = df_work['k_tmp'].map(in_hist.groupby('k_tmp')['수량'].sum()).fillna(0).astype(int)
        ...
        # 에디터 화면 (사장님 요청 항목 모두 포함)
        disp4 = [vendor, item, option, avail, "리오더 수량", "리오더입고수량", "과거 리오더입고수량", t3day, "일판매량", "권장발주량"]
        ...
        # 추가 발주량 세션 연동
        df_final['추가 리오더'] = df_final['k_tmp'].map(st.session_state.extra_order_dict).fillna(0).astype(int)
...
# --- [탭 2: 기록 조회] ---
with tab2:
    st.subheader("📜 6단계: 입출고 및 발주 히스토리")
    hist_all = load_history_from_gsheet()
    if not hist_all.empty:
        c1, c2 = st.columns(2)
        hist_all['날짜'] = hist_all['저장시간'].astype(str).str.split(' ').str[0]
        s_date = c1.date_input("조회 시작일", datetime.now() - timedelta(days=7))
        e_date = c2.date_input("조회 종료일", datetime.now())
        
        mask = (hist_all['날짜'] >= s_date.strftime("%Y-%m-%d")) & (hist_all['날짜'] <= e_date.strftime("%Y-%m-%d"))
        st.dataframe(hist_all[mask], use_container_width=True, hide_index=True)
        
        csv = hist_all[mask].to_csv(index=False).encode('utf-8-sig')
        st.download_button("📥 기록 다운로드 (CSV)", csv, f"history_{datetime.now().strftime('%Y%m%d')}.csv")
    else:
