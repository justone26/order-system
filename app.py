...
# [핵심] 데이터가 있으면 화면을 그린다 (파일 업로드와 관계없이 실행)
if st.session_state.df_raw is not None:
    ...
    edited_df = st.data_editor(df_filtered[display_cols], use_container_width=True, 
                               disabled=[c for c in display_cols if c not in ["1차 리오더", "2차 리오더"]], key="main_editor")
