...
with tab1:
    ...
    # 버튼을 화면 왼쪽 구석에 배치하기 위해, 작은 너비의 컬럼을 만듭니다.
    c_btn, _ = st.columns([1, 4]) # 1:4 비율로, 버튼은 왼쪽 1칸만 차지하게 함
    
    # "🔄 6단계: 이전 데이터 로드" -> "📂 6단계: 이전 데이터 로드" 로 아이콘 변경
    # "📂 " 아이콘은 윈도우 키 + 마침표(.)를 눌러 '폴더'를 검색해서 바꾸실 수 있습니다.
