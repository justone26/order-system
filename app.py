    ...
    if 'df_final' not in st.session_state:
        ...
        with st.form("final_form"):
            edited_df = st.data_editor(
                ...
                key="main_editor", 
                column_config={
                    "상태": st.column_config.TextColumn("상태", width=80, disabled=True),
                    p['vn']: st.column_config.TextColumn("공급처", width=100, disabled=True),
                    p['it']: st.column_config.TextColumn("상품명", width=350, disabled=True),
                    p['op']: st.column_config.TextColumn("옵션", width=120, disabled=True),
                    ...
                    "입고차감": st.column_config.NumberColumn("📥 입고(-)", width=80, min_value=0), 
                    "추가발주": st.column_config.NumberColumn("➕ 발주(+)", width=80, min_value=0),
