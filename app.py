...
if st.session_state.df_raw is not None:
    ...
    # [2. 고정된 매핑 섹션]
    st.subheader("⚙️ 1단계: 자동 매핑 (고정)")
    with st.expander("매핑 설정", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            ...
            vendor_option = st.selectbox("공급처옵션", cols, index=get_idx(cols, ['공급처옵션', '거래처옵션']))
        with c2:
    ...
    st.download_button("📥 최종 데이터 다운로드", buffer.getvalue(), "최종_발주서.xlsx")
