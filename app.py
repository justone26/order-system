...
# ------------------------------------------------------------------
# [6단계: 전체 히스토리 관리] - 상단 회차 선택(Selectbox) 방식
# ------------------------------------------------------------------
if st.session_state.get('analyzed'):
    ...
    # 데이터가 있을 때만 검색과 회차 선택 활성화
    if st.session_state.get('v6_data') is not None:
        df_hist = st.session_state.v6_data.copy()
        ...
        with f4:
            ...
            session_options = [f"{len(sessions)-i}회차 ({t[11:16]} 저장)" for i, t in enumerate(sessions)]
            sel_session_label = st.selectbox("📦 4. 회차 선택", session_options, key="v6_session_select")
            
            # 선택된 회차의 실제 timestamp 추출
            sel_idx = session_options.index(sel_session_label)
            target_time = sessions[sel_idx]

        # [2] 결과 출력 영역
        df_display = df_hist[df_hist["발주시간"] == target_time].copy()
        
        # 상품명 검색 필터 (선택된 회차 내에서 검색)
        if h_q:
