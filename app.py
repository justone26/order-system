        ...
        with st.form("form_v5"):
            ...
            if st.form_submit_button("✅ 5단계 수량 확정 (리오더 합산)"):
                ...
                for r_idx, val in changes.items():
                    ...
                    if "추가발주수량" in val:
                        st.session_state.df_raw.at[o_idx, "리오더 수량"] += int(val["추가발주수량"])
                        st.session_state.add_order_dict[o_idx] = int(val["추가발주수량"])
                st.success("✅ 합산 완료!"); time.sleep(1); st.rerun()

        if st.button("💾 구글 시트 저장 및 6단계 전송", use_container_width=True):
