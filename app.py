...
with tab1:
    ...
    if st.session_state.df_raw is not None:
        ...
        # 1-2단계: 설정
        st.subheader("⚙️ 1-2단계: 매핑 및 파라미터")
        c1, c2 = st.columns(2)
        ...
        lead_time = c2.number_input("리드타임", value=10)
        safety_stock = c2.number_input("안전재고", value=7)
        ...
        # 요청하신 모든 셀(컬럼) 리스트 (존재 여부 체크 후 표시)
        all_cols = [sold_out, vendor, item, option, vendor_opt, stock, avail, "입고예정수량(리오더)", "리오더입고수량", "최근입고기록", t3day, t1week, '권장 발주량']
        ...
        if '권장 발주량' in st.session_state.df_raw.columns:
            ...
            st.download_button("📥 최종 데이터 다운로드", data=output.getvalue(), file_name=f"결과_{datetime.now().strftime('%m%d')}.xlsx")

# --- [🌙 탭 2: 동대문 관리 및 히스토리] ---
with tab2:
    st.subheader("🌙 동대문 사입 및 히스토리")
    # 기존 동대문 로직 그대로 유지...
    ...
    st.divider()
    st.subheader("📜 6단계: 구글 시트 히스토리 전체 조회")
