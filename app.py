...
if st.session_state.df_raw is not None:
    ...
    if '권장 발주량' in st.session_state.df_raw.columns:
        ...
        # 요청하신 핵심 4가지 항목 요약
        summary_cols = [c for c in [vendor, item, avail, '권장 발주량'] if c in to_order.columns]
    ...
    st.download_button("📥 최종 데이터 다운로드", data=buffer.getvalue(), file_name="결과.xlsx", 
