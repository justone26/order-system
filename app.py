...
if st.session_state.get('analyzed') and st.session_state.df_raw is not None:
    ...
    sold_out, item, option = p['so'], p['it'], p['op']
    ...
    # 2. UI 배치 (상태 필터가 맨 앞, 그다음 검색)
    f_c1, f_c2, f_c3 = st.columns([1, 2, 1])
    filter_m = f_c1.selectbox("🚦 상태 필터", ["전체보기", "정상만", "품절만"], index=1, key="v4_filter_final")
    search_q = f_c2.text_input("🔍 상품명/옵션 검색", placeholder="검색어를 입력하세요...", key="v4_search_final")
    hist_date_4 = f_c3.date_input("🗓️ 입고 날짜", datetime.now().date(), key="v4_date_final")
    ...
    # 4. 필터 로직 (품절 텍스트 유무 기준)
    df_work['status_temp'] = df_work[sold_out].astype(str).str.strip()
    is_soldout = df_work['status_temp'].str.contains('품절', na=False)
    ...
    # 사장님 요청 순서: 과거리오더 입고 -> 3일 발주수량 -> 일판매량 -> 권장발주량
    final_cols = [
    ...
    with st.form("v4_form_final"):
        st.data_editor(
            ...
            key="v4_editor_final",
            column_config={
                "과거리오더 입고": st.column_config.NumberColumn(disabled=True),
                "3일 발주수량": st.column_config.NumberColumn(disabled=True),
                "일판매량": st.column_config.NumberColumn(disabled=True),
                "권장발주량": st.column_config.NumberColumn(disabled=True)
            }
        )
