...
if st.session_state.get('analyzed') and st.session_state.df_raw is not None:
    ...
    # ⭐ 금고(session_state)에서 직접 입력했던 수량 다시 매핑 (검색 시 보존용)
    ...
    with f1: 
        # 필터 변경 시에도 현재 입력값 보존을 위해 on_change 추가
    ...
    if curr_q:
        # 검색어가 있으면 필터 상태 무시하고 모든 검색 결과 노출
        df_final_v5 = df_ns[
