...
if st.session_state.df_raw is not None:
    ...
    with c1:
        ...
        vendor_opt = st.selectbox("공급처옵션", cols, index=get_idx(cols, ['공급처옵션']))
    with c2:
        stock = st.selectbox("정상재고", cols, index=get_idx(cols, ['정상재고']))
        avail = st.selectbox("가용재고", cols, index=get_idx(cols, ['가용재고']))
        t3day = st.selectbox("3일 발주 합계", cols, index=get_idx(cols, ['3일']))
        t1week = st.selectbox("1주 발주 합계", cols, index=get_idx(cols, ['1주']))
