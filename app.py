...
# ==========================================================
# --- [5단계: 최종 발주 리스트 요약 (모든 기능 복구 및 완결판)] ---
# ==========================================================
if st.session_state.get('analyzed') and st.session_state.df_raw is not None:
    st.divider()
    st.subheader("📋 5단계: 최종 발주 리스트 요약 (기능 완결 버전)")
    ...
    # 원본 인덱스 유지가 금고(session_state) 데이터 보존의 핵심입니다.
    ...
    # [3. 핵심 계산 로직 (신상품 등록일 보정 포함)]
    # 일판매량 계산 (상단 공통 함수 calc_daily_sales_with_reg_v4 사용)
    ...
    # ⭐ 금고(session_state)에서 입력했던 수량 다시 매핑 (검색 시 수량 유지 핵심)
    ...
    with f1: 
        # 필터 변경 시에도 현재 입력값이 금고에 저장되도록 on_change 추가
        m5_f = st.selectbox("🚦 상태 필터", ["🚨 고위험/주의", "✅ 전체정상"], key="v5_filter_fixed", on_change=sync_all_and_save_mem)
    with f2: 
        # 검색 시 sync_all_and_save_mem를 호출하여 현재 입력값을 금고에 먼저 저장
    ...
    if curr_q:
        # 검색어가 있으면 필터 상태(고위험/주의)를 무시하고 검색 결과 전체 노출
        df_final_v5 = df_ns[
    ...
    with st.form("v5_final_full_form_fixed"):
        ...
        if st.form_submit_button("✅ 최종 발주 확정 (시트1 리오더 수량 합산 저장)", use_container_width=True, type="primary"):
            # 저장 버튼 누를 때 마지막 입력 수치까지 금고에 동기화
            ...
            if any(v > 0 for v in final_dict.values()):
                ...
                st.session_state.add_order_dict = {} # 저장 후 금고 비우기
                st.success("✅ 리오더 수량이 시트에 합산 저장되었습니다!"); time.sleep(1); st.rerun()
            else:
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("💾 발주 기록지에 전체 내역 저장", use_container_width=True):
            # 권장수량 혹은 추가입력이 있는 것만 로그 작성
            df_log = df_display_v5[(df_display_v5['권장수량'] > 0) | (df_display_v5['추가입력'] > 0)].copy()
            if not df_log.empty:
                now_str = datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')
                log_data = [[now_str, str(r[item]), str(r[option]), str(r[v_item]), int(r[avail]), int(r['리오더 수량']), int(r['추가입력']), int(r['권장수량'])] for _, r in df_log.iterrows()]
                get_sheet().worksheet("발주기록").append_rows(log_data)
                st.success("✅ 발주기록 시트에 저장 완료!")
    with c2:
