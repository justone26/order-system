...
if st.session_state.get('analyzed') and st.session_state.df_raw is not None:
    ...
    sold_out, item, option = p['so'], p['it'], p['op'] # 품절컬럼, 상품명, 옵션
    ...
    # 2. UI 배치 변경 (상태 필터가 맨 앞으로)
    f_c1, f_c2, f_c3 = st.columns([1, 2, 1])
    
    # [위치변경 1] 상태 필터 (index=1로 '정상만' 기본값)
    filter_m = f_c1.selectbox("🚦 상태 필터", ["전체보기", "정상만", "품절만"], index=1, key="v4_filter_pos")
    
    # [위치변경 2] 상품명/옵션 검색
    search_q = f_c2.text_input("🔍 상품명/옵션 검색", placeholder="검색어를 입력하세요...", key="v4_search_pos")
    
    # [날짜 선택]
    hist_date_4 = f_c3.date_input("🗓️ 입고 날짜", datetime.now().date(), key="v4_date_pos")

    # 3. 입고 기록 매칭 및 계산 (중략 없이 핵심 로직 포함)
    ...
    # 4. 필터 로직 (사장님 요청: '품절' 텍스트 유무로 구분)
    # 품절 컬럼에 '품절'이라는 단어가 포함되어 있는지 확인
    is_soldout_text = df_work[sold_out].astype(str).str.contains('품절', na=False)

    if filter_m == "정상만":
        df_filtered = df_work[~is_soldout_text] # '품절' 글자가 없는 것들
    elif filter_m == "품절만":
        df_filtered = df_work[is_soldout_text]  # '품절' 글자가 있는 것들
    else:
        df_filtered = df_work
    ...
    df_display = df_filtered.rename(columns={
        sold_out: "품절상태", vendor: "공급쳐", v_item: "공급쳐 상품명", 
        item: "상품명", option: "옵션", stock: "정상재고", avail: "가용재고"
    })
    final_cols = ["품절상태", "공급쳐", "상품명", "옵션", "공급쳐 상품명", "정상재고", "가용재고", "리오더 수량", "리오더 입고수량", "과거리오더 입고", "일판매량", "권장발주량"]

    # 6. 데이터 에디터 및 저장 폼
    with st.form("v4_final_form"):
        st.data_editor(
            df_display[final_cols] if not df_display.empty else pd.DataFrame(columns=final_cols),
    ...
    if df_display.empty:
        st.info("💡 표시할 데이터가 없습니다. 필터나 검색어를 확인해주세요.")
