...
with tab1:
    ...
    if st.session_state.df_raw is not None:
        ...
        reg_date = c1.selectbox("상품등록일", cols, index=get_idx(cols, ['등록일', '상품등록']))
        ...
        if l3.button("📊 분석 시작", use_container_width=True):
            df = st.session_state.df_raw
            df['일판매량'] = (pd.to_numeric(df[t3day], errors='coerce').fillna(0) / 3).round(1)
            df['권장 발주량'] = ((df['일판매량'] * (lead_time + safety_stock)) - 
        ...
        # 컬럼 순서 재구성 (원본 순서 유지 + 입고 관련 셀 삽입)
        display_cols = []
        for c in st.session_state.df_raw.columns:
            display_cols.append(c)
            if c == "입고예정수량(리오더)":
                if "리오더입고수량" not in display_cols: display_cols.append("리오더입고수량")
                if "최근입고기록" not in display_cols: display_cols.append("최근입고기록")
        ...
        def on_edit():
            ...
            for idx_str, change in changes.items():
                ...
                if "리오더입고수량" in change:
                    val = int(change["리오더입고수량"])
                    st.session_state.df_raw.at[orig_idx, "입고예정수량(리오더)"] -= val
                    # 구글 시트 history 기록
                    sh = get_sheet()
                    sh.worksheet("history").append_row([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 
                                                       str(df_disp.at[orig_idx, item]), 
                                                       str(df_disp.at[orig_idx, option]), val])
            # 구글 시트 Sheet1 동기화
            sh = get_sheet()
        ...
        st.data_editor(df_disp[display_cols], use_container_width=True, key="editor", on_change=on_edit)
        ...
        if '권장 발주량' in st.session_state.df_raw.columns:
            ...
            st.dataframe(to_order[display_cols], use_container_width=True)
            
            output = BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                to_order.to_excel(writer, index=False)
            st.download_button("📥 발주서 다운로드", data=output.getvalue(), file_name=f"발주서_{datetime.now().strftime('%m%d')}.xlsx")

# --- [🌙 탭 2: 동대문 사입 및 히스토리] ---
with tab2:
    st.subheader("📜 6단계: 입고 히스토리 (구글 시트)")
    if st.button("🔄 히스토리 기록 새로고침"):
        sh = get_sheet()
        if sh:
    ...
    if dong_file:
        ...
        # 동대문 사입 편집기
        st.data_editor(df_dong, use_container_width=True, key="dong_editor")
